...
module.exports = {
  ...
  userProfile : async (req, res) => {
    ...
    getRefferelPatient :  async (req, res) =>{
    	...
    	let providers = await patientRefferel.aggregate().match({patient_name:{$ne:''}})
          .project({
    ...
    getPatientDose : async (req, res) =>{
      let Dose = await pateintRefferelDose.find({}).sort({createdAt:-1});
        if(Dose) {
  ...
  getVideos : async (req, res) =>{
        ...
        let getVideos = await educationVideo.find({}).sort({_id:-1});
         if(getVideos) {
   ...
   getServey : async (req, res) =>{
    ...
    let getServeys = await Servey.find({}).sort({createdAt:-1});
   ...
   notificationSave: async (req, res) =>{
    ...
    getNotification: async (req, res) =>{
        let result = await Notifications.find({}).sort({createdAt:-1});
    ...
    addPages : async (req, res) =>{
        ...
        if(data){
            ...
            res.json({status:0, message:'Need any Params Like about or terms_conditions.'})
        }
    },
